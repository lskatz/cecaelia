...
sub testForUnevenness{
  ...
  # Cache taxonkit results: this will be modified by reference
  ...
  while(my $line = <$krakenFh>){
    ...
    my @sortedTaxids = sort {$taxidCount{$b} <=> $taxidCount{$a}} keys(%taxidCount);
    my $majorityTaxid = $sortedTaxids[0];
    if($majorityTaxid == 0){
...
# Gets taxids of the kraken line at the target rank
sub findTaxIds{
  my($line, $targetRank, $taxonkitLineageCache, $settings) = @_;
